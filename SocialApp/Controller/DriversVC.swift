...
class DriversVC: UIViewController {
    ...
    func updateList(completion: (()->Void)!) {
        DataService.ds.refDrivers.observeSingleEvent(of: .value) { (snapshot) in
            self.drivers.removeAll()
            if let snapshot = snapshot.children.allObjects as? [DataSnapshot] {
                for snap in snapshot {
                    if let userData = snap.value as? Dictionary<String, Any> {
                        let uid = snap.key
                        let driver = OtherUser.init(uid, userData)
...
extension DriversVC: UITableViewDelegate, UITableViewDataSource, UITextFieldDelegate {
